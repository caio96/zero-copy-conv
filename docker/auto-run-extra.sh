...
# The scalability thread counts (1, 2, 4 and 8) and the core pinning used by
# the scalability and memory runs are fixed and cannot be configured here.
...
#
# Plot the scalability figure last: plotting is presentation only, and running
# it here keeps a plotting failure from aborting the experiments above.
#

python3 "${WORKDIR}/scalability/plot_scalability.py" \
    --data "${HOME}/results/scalability/data.csv" \
    --output "${HOME}/results/scalability/scalability.png"
