...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--filter-models",
        type=str,
        help="Path to a file where each line contains a model name (generated by filter_models.py). Only models in that file will be run.",
    )
    ...
    if filter_file:
        ...
        with open(filter_file) as f:
            filter_models = {line.strip() for line in f.read().splitlines() if line.strip()}
        if not filter_models:
            print(f"Filter file {filter_file} is empty, no models to run.", file=sys.stderr)
            sys.exit(1)
