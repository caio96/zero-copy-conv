...
void conv_2d_naive(float *__restrict__ input, float *__restrict__ output,
  ...
  for (int b = 0; b < batch; ++b) {
    float *single_input = &input[b * input_channels * input_height * input_width];
    float *single_output = &output[b * output_channels * output_height * output_width];
    ...
    for (int oc = 0; oc < output_channels_per_group; ++oc) {
      ...
      for (int oh = 0; oh < output_height; ++oh) {
        for (int ow = 0; ow < output_width; ++ow) {
            ...
            for (int fh = 0; fh < filter_height; ++fh) {
              for (int fw = 0; fw < filter_width; ++fw) {
                ...
                if (is_a_ge_zero_and_a_lt_b(ih, input_height) &&
                  ...
                  int input_idx = (ic_group * input_height + ih) * input_width + iw;
                  int filter_idx = ((oc_group * input_channels_per_group + ic) * filter_height + fh) * filter_width + fw;
          ...
          // Output index
          int output_idx = (oc_group * output_height + oh) * output_width + ow;
