...
void conv_2d_zero_copy(float *__restrict__ input,
...
void conv_2d_zero_copy_main(float *__restrict__ input, float *__restrict__ output,
  ...
  if (DH == 1 && DW == 1 && GR == 1) {
    conv_2d_zero_copy(input, output, filters, N, H, W, C, FH, FW, OH,
                              OW, M, PH, PW, SH, SW);
  } else {
    conv_2d_zero_copy_ext(input, output, filters, N, H, W, C, FH, FW, OH, OW,
