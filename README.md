...
Workflow:
 - Run `timm_convolution_extraction` to generate a pickle file containing convolution parameters, or use the pickle file provided in `data`
 - Run `pickle_to_filtered_csv` to generate a csv based on the pickle file and to filter the desired convolution types
 - Build this repo with the instructions above
 - Run `benchmark_runner` with the build dir and the generated csv to test performance or correctness
 - Run `summarize_correctness` or `summarize_performance` depending on the type of run with the output csv generated by the runner to get final csv and graphs
