...
PROFILE="false"
PROFILE_OUTPUT=""
# Perf events used by --save-profile. Override by exporting PERF_EVENTS.
# The value must be a comma-separated event list with no spaces.
PERF_EVENTS="${PERF_EVENTS:-cpu_core/cycles/,cpu_core/instructions/,mem_load_retired.l1_hit,mem_load_retired.l1_miss,mem_load_retired.l2_hit,mem_load_retired.l2_miss,mem_load_retired.l3_hit,mem_load_retired.l3_miss,cpu_core/dtlb_load_misses.walk_completed/,page-faults,fp_arith_inst_retired.scalar,fp_arith_inst_retired.vector}"
...
if [[ $RERUN == "true" ]]; then
  ...
  if [[ $header != "conv_parameters,rerun_methods" ]]; then
    echo "ERROR: CSV file does not contain the expected header. This csv is generated by the summarize_performance.py script."
    exit 1
  fi
fi
...
if [[ "$CHECK_CORRECTNESS" == "false" ]] && [[ ${#executables[@]} -eq 0 ]]; then
  echo "ERROR: No runnable benchmark_* executables found in $BUILD_DIR"
  exit 1
fi

EXE_DIR=""
if [[ ${#executables[@]} -gt 0 ]]; then
  EXE_DIR=$(dirname "${executables[0]}")
fi

CORRECTNESS_EXECUTABLE=$(find "$BUILD_DIR" -type f -name correctness | head -n1)
if [[ "$CHECK_CORRECTNESS" == "true" ]] && [[ ! -f "$CORRECTNESS_EXECUTABLE" ]]; then
  echo "Correctness executable not found. Maybe the name changed?"
  echo "Looking for an executable named 'correctness' under $BUILD_DIR"
  exit 1
fi
...
if [[ "$PARALLEL_SINGLE_THREAD_MODE" == "true" ]]; then
  ...
  export CHECK_CORRECTNESS BATCH_SIZE CORRECTNESS_EXECUTABLE OUTPUT_LOG RERUN BUILD_DIR EXE_DIR
        ...
        if [[ "$RERUN" == "true" ]]; then
          ...
          for method in $(echo "$rerun_methods" | tr "," "\n"); do
            ...
            if [[ "$method" == "Naive" ]]; then
              executable="$EXE_DIR/benchmark_naive"
            elif [[ "$method" == "Im2col" ]]; then
              executable="$EXE_DIR/benchmark_im2col"
            elif [[ "$method" == "Yaconv" ]]; then
              executable="$EXE_DIR/benchmark_yaconv"
            elif [[ "$method" =~ ^ZeroCopy.*$ ]]; then
              executable="$EXE_DIR/benchmark_zero_copy"
            elif [[ "$method" =~ ^LibTorch_ZeroCopy2D.*$ ]]; then
              executable="$EXE_DIR/benchmark_libtorch_zerocopy"
            elif [[ "$method" == "LibTorch" ]]; then
              executable="$EXE_DIR/benchmark_libtorch"
            else
...
for repeat in $(seq "$REPEATS"); do
  ...
  while IFS=',' read -r conv_parameters occurrences models; do
    ...
    PERF_COMMAND=""
    if [[ "$PROFILE" == "true" ]]; then
      PERF_COMMAND="perf stat -x, -r${PERF_REPEATS} -e "$PERF_EVENTS" -o $PROFILE_OUTPUT --append"
    fi
