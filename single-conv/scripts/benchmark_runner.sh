...
function Help()
{
   ...
   echo -e "\t--rerun: Specify the csv file that contains 'conv_parameters' and 'non_confident_methods', only these configurations will be run."
...
# Parse Arguments
PARSED_ARGUMENTS=$(getopt -a -n "benchmark_runner" -o hv --long append-output,parallel-single-thread-mode,check-correctness,repeats:,batch-size:,threads:,core-range:,rerun -- "$@")
if [ $? -ne 0 ]; then
...
if [[ $RERUN == "true" ]]; then
  ...
  if [[ $header != "conv_parameters,non_confident_methods" ]]; then
...
if [[ "$PARALLEL_SINGLE_THREAD_MODE" == "true" ]]; then
  ...
  numactl --physcpubind "$CORE_RANGE" parallel -a "$CSV_FILE" --skip-first-line --colsep "," --joblog "$parallel_log" --bar -j "$OMP_NUM_THREADS" '
      ...
      if [[ "$CHECK_CORRECTNESS" == "true" ]]; then
        ...
        if [[ "$RERUN" == "true" ]]; then
          non_confident_methods={2}
          for method in $(echo "$non_confident_methods" | tr "," "\n"); do
...
for repeat in $(seq "$REPEATS"); do
  ...
  while IFS=',' read -r conv_parameters occurrences models; do
    ...
    if [[ "$RERUN" == "true" ]]; then
      non_confident_methods="$occurrences"
      for method in $(echo "$non_confident_methods" | tr "," "\n"); do
        # Choose executable based on method to rerun
        if [[ "$method" == "Naive" ]]; then
          executable="$BUILD_DIR/benchmark_naive"
